...
import { Video, Mail, Lock, LogIn, Sparkles, Loader2, ShieldAlert } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    if (IS_MOCK) {
      ...
      try {
        ...
        setError("Failed to authenticate in mock mode.");
      } finally {
        setIsLoading(false);
      }
      return;
    ...
    try {
      ...
      await fetchProfile(u.uid, u.displayName || u.email?.split("@")[0], u.email || "");
      router.push("/dashboard");
    } catch (err: any) {
      console.error(err);
  ...
  const triggerMockLogin = async () => {
    ...
    try {
      ...
      setError("Failed to log in with Mock User");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="flex-1 flex items-center justify-center py-16 px-4 sm:px-6 lg:px-8 bg-glow-primary">
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="text-center flex flex-col items-center">
          ...
          <h2 className="text-2xl font-bold tracking-tight text-white">Sign in to Viddy</h2>
          <p className="text-sm text-zinc-400 mt-1.5">Understand every video instantly</p>
        </div>
        ...
        <div className="rounded-2xl border border-white/8 glass-panel p-6 shadow-2xl space-y-6">
          {IS_MOCK && (
            <div className="rounded-lg border border-amber-500/10 bg-amber-500/5 p-4 flex gap-3 text-amber-200 text-xs">
              ...
              <div>
                <p className="font-bold mb-1">Firebase Configuration is Missing</p>
                <p className="text-zinc-400 mb-2 leading-relaxed">
                  The client environment variables are not set. The app is running in developer mock mode.
                </p>
                <button
        ...
        <p className="text-center text-xs text-zinc-500">
          ...
          <Link href="/signup" className="text-primary hover:text-white font-semibold transition-colors">
            Sign Up
          </Link>
        </p>
